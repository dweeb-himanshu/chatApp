          ...
          if(isSenderBlocked || $scope.isReceiverBlocked){
            ...
            Meteor.call('updateRecentMessage', groupMessage, $scope.paramDetails.user_id, $rootScope.userInfo.user_id, $scope.enterMessage, imageId,
             angular.toJson($scope.paramDetails), angular.toJson($scope.userInfo), connectionStatus);
              ...
              Meteor.call('sendPushNotification', $scope.paramDetails.imei,$scope.enterMessage);
          }
     ...
     function subscribeMessages(){
        if($scope.individualChatFlag){
         $scope.helpers({
           ...
           {
             $and: [
                {"to": $rootScope.userInfo.user_id},
                 {"from": $scope.paramDetails.user_id},
               {"groupMessage": false}
             ]
           }
         ]
...
$scope.filterGenres = function(msg) {
    ...
    */
    ...
    $scope.downloadImage = function(imageId){
      ...
      $scope.obj = {};
      $scope.obj = Meteor.subscribe('images', imageId);
      if($scope.obj !== null && $scope.obj !== undefined){
          $scope.isImageAvail = true;
      }
      $scope.helpers({
           imageDownloads: () => {
      ...
      $rootScope.DownloadImageProperty = $scope.imageDownloads[0];
       $ionicPopup.alert({
             templateUrl: 'client/templates/downloadedImage.html'
           })
          .then(function(res) {
